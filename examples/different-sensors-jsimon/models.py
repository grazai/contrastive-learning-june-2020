...
class Projection(nn.Module):
    ...
    def __init__(self, ls: List[int]):
        ...
        for i in range(1, len(ls)-1):
            l_in, l_out = ls[i-1], ls[i]
            lin = nn.Linear(l_in, l_out, bias=False)
            relu = nn.ReLU()
            lays.append(lin)
            lays.append(relu)
        l_in, l_out = ls[-2], ls[-1]
        lin = nn.Linear(l_in, l_out, bias=False)
        lays.append(lin)
...
class SimpleEncoder(nn.Module):
    ...
    def __init__(self):
        ...
        self.encoding = nn.Conv1d(2, 2, 10, stride=2)

        self.layers = nn.Sequential(
            nn.Conv1d(2, 2, 10, stride=2),
            nn.ReLU(),
            nn.Conv1d(2, 1, 10, stride=2),
            nn.ReLU()
        )
...
# Inception model taken from https://github.com/okrasolar/pytorch-timeseries
